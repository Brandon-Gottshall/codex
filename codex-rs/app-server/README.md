...
- `desktop/thread/route` — request that a desktop host route its visible selection to a thread id. The open-source app-server validates that the thread target exists, then returns `routed: false` with `authority: "validatedOnly"` unless a native desktop host adapter is present. Clients must not treat this as proof of GUI selection without a routed response and readback.
- `desktop/thread/selection/read` — read back the desktop host's selected thread when a native host adapter supports it. Without that adapter, the open-source app-server returns `selection: null` and `authority: "unsupported"`.
