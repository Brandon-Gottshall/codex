...
- `desktop/thread/route` — request that a desktop host route its visible selection to a thread id. The app-server validates that the target thread exists and, when an initialized desktop host client is connected, forwards a server request to that host. The response is trusted only when the host reports `authority: "nativeQuietRoute"`, `routed: true`, matching selection readback, and `focused: false` for quiet routes. Otherwise the app-server returns `routed: false` with `authority: "validatedOnly"`.
- `desktop/thread/selection/read` — read back the desktop host's selected thread when a connected native host supports it. Without that adapter, or when the host response is not trusted, the app-server returns `selection: null` and `authority: "unsupported"`.
